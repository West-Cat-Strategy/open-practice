...
describe("migration integrity parity", () => {
  it("passes when SQL files and journal entries match exactly", () => {
    ...
    assert.equal(formatParityReport(report), "Migration parity passed: 2 SQL files match 2 journal entries.");
  });
...
describe("migration integrity replay", () => {
  ...
  it("cleans up the replay database when migration fails", () => {
    ...
    assert.equal(calls.at(-1).args.at(-1), 'DROP DATABASE IF EXISTS "open_practice_migration_replay_test" WITH (FORCE)');
  });
  ...
  it("parses replay CLI options", () => {
    assert.deepEqual(parseArgs(["--mode", "replay", "--admin-client", "docker", "--keep-database"]), {
      mode: "replay",
      databaseUrl: null,
      databaseName: null,
      keepDatabase: true,

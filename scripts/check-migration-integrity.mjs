...
export function collectSqlMigrations({
...
function validateIndexSequence(items, { label, failures }) {
  ...
  if (missing.length > 0 || outOfRange.length > 0) {
    ...
    failures.push(
      `${label} indexes must be contiguous from 0 through ${expectedLength - 1}; ${parts.join("; ")}.`,
    );
  }
}
...
function validateJournalEntries(entries, failures) {
  ...
  for (const entry of entries) {
    if (entry.idx !== entry.position) {
      failures.push(
        `Journal entry ${entry.position} has idx ${entry.idx}; entries must be ordered by idx.`,
      );
    }
    ...
    if (tagIndex !== entry.idx) {
      failures.push(
        `Journal entry ${entry.position} tag ${entry.tag} does not match idx ${entry.idx}.`,
      );
    }
  }
...
function validateSqlMigrations({ migrations, invalidNames }, failures) {
  if (invalidNames.length > 0) {
    failures.push(
      `Unrecognized SQL migration ${pluralize(invalidNames.length, "name")}: ${invalidNames.join(", ")}.`,
    );
  }
  ...
  for (const migration of migrations) {
    ...
    if (tagIndex !== migration.idx) {
      failures.push(
        `SQL migration ${migration.file} tag prefix does not match parsed idx ${migration.idx}.`,
      );
    }
  }
}
...
function validateSqlJournalParity(migrations, entries, failures) {
  ...
  if (missingJournalEntries.length > 0) {
    failures.push(
      `SQL/journal drift: SQL files missing journal entries: ${missingJournalEntries.join(", ")}.`,
    );
  }

  if (missingSqlFiles.length > 0) {
    failures.push(
      `SQL/journal drift: journal entries missing SQL files: ${missingSqlFiles.join(", ")}.`,
    );
  }
...
function resolveReplayDatabaseUrl(input, env = process.env) {
  return (
    input ?? env.MIGRATION_REPLAY_DATABASE_URL ?? env.DATABASE_URL ?? DEFAULT_LOCAL_DATABASE_URL
  );
}
...
export function runMigrationReplay({
  ...
  const resolvedAdminClient = chooseAdminClient({
    requestedClient: adminClient,
    spawn,
    cwd: rootDir,
  });
  const cleanupSql = [
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--mode") {
      ...
      if (mode !== "parity" && mode !== "replay")
        throw new Error("--mode must be parity or replay.");
      options.mode = mode;
      continue;
    }

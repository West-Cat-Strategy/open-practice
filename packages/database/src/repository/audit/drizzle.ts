...
export async function listDrizzleFilteredAuditEvents(
  ...
  if (filter.resourceType) {
    clauses.push(eq(schema.auditEvents.resourceType, filter.resourceType));
  }
  if (filter.resourceId) {
    clauses.push(eq(schema.auditEvents.resourceId, filter.resourceId));
  }
  if (filter.matterId) {
    clauses.push(auditEventMatterFilter(filter.matterId));
  }

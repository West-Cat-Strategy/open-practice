  ...
  const base: TrustControlsDashboardResponse = {
    ...
    balanceSnapshotComparison: {
      ...
      latestReconciliationPreview: {
        importBatchId: "statement-import-batch-001",
        accountId: "acct-trust-bank",
      ...
      reviewReasons: ["reconciliation_variance", "unmatched_statement_rows"],
      policy: {
...
describe("dashboard client behavior", () => {
  ...
  it("summarizes read-only trust controls for the Funds workbench", async () => {
    ...
    expect(controls.balanceSnapshotComparison).toMatchObject({
      reviewOnly: true,
      currentTrustBalance: {
        totalCents: 150000,
        balanceCount: 1,

...
export type HomeSearchParams = Promise<Record<string, string | string[] | undefined>>;
...
export async function renderOpenPracticeHome({
  canonicalSection,
  searchParams,
}: {
  canonicalSection?: DashboardNavigationSectionKey;
  searchParams?: HomeSearchParams;
}) {
  const resolvedSearchParams = searchParams ? await searchParams : {};
  const requestedSection = canonicalSection ?? firstSearchParam(resolvedSearchParams.section);
  const requestedMatterId = firstSearchParam(resolvedSearchParams.matter);
  ...
  return (
    <DashboardClient
      ...
      initialMatterId={requestedMatterId ?? undefined}
      ...
      matters={matters}
      navigationMode={canonicalSection ? "path" : "query"}
      overview={overview}

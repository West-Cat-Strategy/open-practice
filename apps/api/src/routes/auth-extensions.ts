...
export function registerAuthExtensionRoutes(
  ...
  server.get("/api/auth/extensions", async (request) => {
    ...
    const [account, credentials, recoveryCodes, currentUser] = await Promise.all([
      options.repository.getAuthAccount(request.auth.firmId, request.auth.user.id),
      options.repository.listWebAuthnCredentials(request.auth.firmId, request.auth.user.id),
      options.repository.listRecoveryCodes(request.auth.firmId, request.auth.user.id),
      options.repository.getUser(request.auth.firmId, request.auth.user.id),
    ]);

    const user = currentUser ?? request.auth.user;

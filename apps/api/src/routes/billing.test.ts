...
describe("billing routes", () => {
  ...
  it("matches refund and chargeback review fixtures across staff, auditor, and external users", async () => {
    const repository = new InMemoryOpenPracticeRepository();
    ...
    const fixtureIds = authorizationFixtureCases
      .filter((item) => item.family === "payment_import_refund_chargeback_review")
      .map((item) => item.id);
    expect(fixtureIds).toEqual([
    ...
    await repository.createPaymentImportReviewRecord(
      paymentImportReviewRecord({
        id: assignedListCase.resourceId,
        matterId: assignedListCase.matterId,
        eventFamily: "payment",
        eventStatus: "refund_observed",
        externalPaymentId: "pay_synthetic_refund_auth_assigned",
      }),
    );
    const server = testServer({ repository });

    const assignedCreate = await server.inject({
      method: "POST",
      url: `/api/billing/payment-import-review-records/${assignedListCase.resourceId}/refund-chargeback-reviews`,
      headers: authHeaders(assignedCreateCase.subjectId),
      payload: {
        id: assignedCreateCase.resourceId,
        decision: "exception_confirmed",
        reason: "refund_observed",
        idempotencyKey: "synthetic-refund-chargeback-auth-assigned",
      },
    });
    expect(assignedCreate.statusCode).toBe(200);
    expect(assignedCreate.json()).toMatchObject({
      review: { id: assignedCreateCase.resourceId, category: "refund" },
    });
    ...
    const auditorList = await server.inject({
      method: "GET",
      url: `/api/billing/payment-import-review-records/${auditorListCase.resourceId}/refund-chargeback-reviews`,
    ...
    const beforeDeniedCreates = (
      await repository.listPaymentImportRefundChargebackReviews(firmId, {
        paymentImportReviewRecordId: assignedListCase.resourceId,
      })
    ).length;
    ...
    const portalCreate = await server.inject({
      method: "POST",
      url: `/api/billing/payment-import-review-records/${assignedListCase.resourceId}/refund-chargeback-reviews`,
      headers: authHeaders(portalCreateCase.subjectId),
      payload: {
        id: portalCreateCase.resourceId,
        decision: "exception_confirmed",
        reason: "refund_observed",
        idempotencyKey: "synthetic-refund-chargeback-auth-portal-denied",
      },
    });
    expect(portalCreate.statusCode).toBe(403);
    expect(portalCreate.json()).toMatchObject({ message: "Staff access required" });
    await expect(

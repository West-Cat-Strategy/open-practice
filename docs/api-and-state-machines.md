...
`GET /api/contacts/review-queue` is a redacted operator projection of the same review signals; it
keeps identifier values and duplicate matched values out of the queue payload, stays matter-scope
filtered, and exists for manual review, not automatic contact merging.
...
governance and enqueue surfaces are implemented. The workbench also exposes review-queue counts for
external-upload review, duplicate candidates, supersession, and failed scans without automatic
classification or deletion. Job metadata must not carry email bodies, portal

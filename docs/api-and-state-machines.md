...
| `POST /api/intake-sessions/:id/generated-packages`                                | Generated document records, package runtime replay proof, and SMTP availability summary for an eligible embedded intake package.                                                 |
...
| `POST /api/intake-form-links`                                                     | Creates an expiring token-hashed client form link and returns the raw token plus public portal URL once.                                                                         |
...
| `GET /api/calendar/events?matterId=&startsAfter=&startsBefore=`                   | Matter-scoped operator-entered calendar events plus CalDAV, iCalendar subscription URLs, and meeting-boundary status for the matter dashboard.                                   |
...
| `POST /api/calendar/events/:eventId/invitations`                                  | Queues attendee invitation email only after confirmation or marks invitations skipped/disabled when email or meeting-link capability is unavailable.                             |
...
## Practice Presets And Dashboard UX

First-run setup may include `selectedPresetIds`, currently drawn from clean-room operational starter
presets for general Canada, BC residential tenancy, BC notarial, and Canada small-business records.
Preset selection is customizable: the setup UI fills practice areas, practitioner jurisdictions, and
first-matter defaults, but the submitted `settings.practiceAreas`, practitioner profile, and optional
first matter remain the persisted source of truth.

When setup completes, Open Practice seeds the basic drafting templates plus the selected preset draft
and embedded intake templates. Preset-backed templates store metadata such as
...
provided, `deliveryConfirmation` for one email recipient is required before the create flow can
queue one outbox email while the raw token is still available. List and revoke responses never
...
`notificationEmail` is provided, `deliveryConfirmation` for one email recipient is required before
the create flow can queue one outbox email while the raw token is still available. List and revoke
responses never expose token hashes. Public upload-intent requests resolve the supplied token to its
hash, reject missing, revoked, expired, or fully used links, atomically claim one upload use, create
a document upload intent, and return only the signed PUT URL plus minimal document status fields.
Public completion reuses the existing checksum and scan-state completion rules after verifying the
token and document scope. Granted and denied public reads/writes are recorded in access logs when
...
`signature.requested` outbox email to the signer addresses after delivery confirmation matches the
signer count. Provider events update the request status, set completion or decline timestamps for
terminal statuses, and preserve terminal statuses against out-of-order non-terminal events. Embedded
signature events capture signer consent text, actor ID, IP, user-agent, timestamps, and
caller-provided evidence. Legacy `docuseal` requests remain historical records and are rejected by
embedded event routes.
...
plain-text bodies. Worker attempt, next-retry, terminal-failure, and provider provenance are
recorded in email event metadata and mirrored into outbox delivery-state fields. Manual retry is
allowed only for failed outbox records and requires matching confirmation for the persisted email
recipient count; it appends a queued event with retry provenance, creates a fresh email job
...
authenticated user after delivery confirmation for one recipient. Legacy `docassemble`
templates/sessions are rejected for new generation paths.
...
reconciliation exceptions, unreconciled accounts, recent postings, and invariant diagnostics for
operator review. It does not post ledger entries, approve transactions, create reconciliations,
place holds, add accounting dimensions, or claim compliance-pack coverage.
...
Redis/BullMQ queue access, matching `deliveryConfirmation`, and at least one non-empty text or HTML
body. The route creates the outbox record, queued email event, durable email job lifecycle record,
and BullMQ email job together before returning `queued`. The audit event records IDs,
template/provider, recipient count, matter scope, and related resource references only; email bodies
stay in the outbox record rather than job or audit metadata. The worker reads message bodies from
the outbox record, marks delivery `sent` or `failed`, appends provider result events, and advances
the matching job lifecycle row to `active`, terminal success, retry failure, dead letter, or skipped
status.
...
helper only after their route-specific delivery confirmation is accepted; share and external-upload
notification emails are create-time only because raw tokens are not recoverable after the response.
Calendar attendees are stored as matter-scoped event children with required/optional role, response
status, and invitation state. Invitation attempts are optional: when SMTP or queue delivery is
unavailable, the API records a skipped attendee invitation state without failing attendee
management. Meeting links and guest-token access remain disabled until an explicit meeting
provider/configuration is present; invitation requests that require unavailable meeting-link
capability are rejected before tokenized guest access or provider links are issued. Calendar audit
metadata records event, attendee, email, job, meeting-boundary status, and count identifiers only;
invitation message bodies remain in the outbox record.

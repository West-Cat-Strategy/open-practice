...
signals, and redacted conflict-check history. Quality review can flag likely duplicate candidates,
protected-party handling, portal-sensitive contacts, and manual conflict-check revalidation prompts.
Conflict history exposes only check ID, created time, disposition, matched contact ID, visible
matched matter IDs, match count, and maximum severity; it does not expose raw query snapshots,
prospective names, matched values, contact notes, or inaccessible matters. The dashboard can prefill
the existing audited conflict-check panel from a selected dossier, but the dossier surface does not
add contact editing, duplicate merges, conflict disposition edits, automatic conflict changes, or

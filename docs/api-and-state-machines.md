...
| `GET /api/billing/dashboard`                                                          | Billing dashboard payload for approved unbilled work, draft capture rows, draft-only timer policy metadata, review-only expense category profiles, draft invoices, issued balances, payments, period-lock summaries, and rate-rule summaries.                                                                                                                            |
...
timeline, phone/text note placeholders, and draft-only client-update requests derived from existing
conversation message records. It does not expose email bodies, parsed text, raw storage keys,

...
| `POST /api/portal/intake-forms/:token/items/:itemId/documents/:id/complete`           | Public storage-verified checksum completion for an intake upload item document, returning generic upload receipt state rather than checksum/duplicate details.                                                                                                                                                                                                           |
...
| `GET /api/billing/dashboard`                                                          | Billing dashboard payload for approved unbilled work, draft/submitted capture-review rows, review-only expense category profiles, draft invoices, issued balances, payment request shells, manual payment evidence flags, period-lock summaries, and rate-rule summaries.                                                                                                |
...
| `POST /api/portal/external-uploads/:token/documents/:id/complete`                     | Public token-scoped storage-verified checksum completion for a document upload intent, returning generic upload receipt state rather than checksum/duplicate details.                                                                                                                                                                                                    |

...
| `POST /api/auth/register/options`                                                 | Creates a SimpleWebAuthn passkey registration challenge for the current embedded-auth user.                                                                                                                                                                        |
| `POST /api/auth/register/verify`                                                  | Verifies and stores a passkey credential for the current embedded-auth user.                                                                                                                                                                                       |
| `POST /api/auth/login/options`                                                    | Creates a passkey login challenge for a configured firm/user and RP ID/origin.                                                                                                                                                                                     |
| `POST /api/auth/login/verify`                                                     | Verifies a passkey assertion and creates an embedded session.                                                                                                                                                                                                      |
...
| `GET /api/providers/status`                                                       | Owner/auditor-visible read-only provider posture projection over provider settings, object storage, producer/worker queues, redacted job summaries, email, inbound email, external uploads, document processing, draft assist, and embedded-auth extension status. |
...
The unified provider-status route is read-only and reports those deferred seams as posture only; it
does not add enqueue endpoints, provider setup, live health probes, or worker processors.
...
The Queues operator surface also consumes `GET /api/providers/status` as a compact provider posture
panel for object storage, producer/worker queues, outbound email, inbound email, document
processing, external uploads, and draft assist. It is read-only status context; it does not expose
provider secrets, raw job errors, message bodies, storage keys, portal tokens, or provider setup
controls.
...
`GET /api/providers/status` composes the existing read-only status seams into one operator
projection. It reports firm provider settings without encrypted config, object-storage and
token-signing posture without bucket or credential details, producer and worker queue configured
state, reserved/deferred worker queues, redacted latest job runs, document-processing status, SMTP
email status, inbound-email address posture, external-upload status, draft-assist status, and
embedded-auth extension status. Its `liveHealth` value is explicitly `not_checked`; callers must not
treat it as a provider ping, Redis liveness probe, S3 reachability probe, SMTP test, or AI model
execution check.

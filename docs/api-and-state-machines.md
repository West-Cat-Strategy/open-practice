...
| `GET /api/contacts/review-decisions`                                                  | Lists append-only contact quality review decisions visible through the current user's contact dossier scope.                                                                                                                                                                                       |
| `POST /api/contacts/:contactId/review-decisions`                                      | Records a reviewer decision against a current visible duplicate, protected-party, or conflict-revalidation signal without merging contacts, rewriting contact records, or mutating conflict-check dispositions.                                                                                    |
...
| `POST /api/operational-views/definitions`                                             | Create an owner-private saved operational view definition for a permitted dashboard surface; saved matter views support the dashboard's matter follow-up, overdue filing, uncontacted intake, and expiring upload-link preset families.                                                            |
| `PATCH /api/operational-views/definitions/:id`                                        | Update an owner-private saved operational view definition after rechecking permission scopes.                                                                                                                                                                                                      |
...
| `GET /api/billing/rate-presets`                                                       | Lists firm-wide or matter-scoped billing rate presets visible to the current billing reader.                                                                                                                                                                                                       |
| `POST /api/billing/rate-presets`                                                      | Creates a staff-only billing rate preset with effective dates; time-entry creation stores the selected rate as an immutable `rateCents` snapshot.                                                                                                                                                  |
| `GET /api/billing/period-locks`                                                       | Lists active or released firm/matter billing-period locks visible to the current billing reader.                                                                                                                                                                                                   |
| `POST /api/billing/period-locks`                                                      | Creates a staff-only billing-period lock that blocks time and expense create/update/status-transition attempts inside the locked date range.                                                                                                                                                       |
| `POST /api/billing/period-locks/:id/release`                                          | Releases an active billing-period lock while preserving the original lock record and redacted audit metadata.                                                                                                                                                                                      |
...
| `POST /api/billing/export-requests`                                                   | Creates a staff-only async billing or trust export request with job/audit metadata limited to IDs, kind, status, and record counts; export bodies stay out of job metadata.                                                                                                                        |
| `GET /api/billing/export-requests/:exportJobId`                                       | Reads one async billing or trust export request status after the same billing/trust export authorization check.                                                                                                                                                                                    |
| `GET /api/billing/export-requests/:exportJobId/download`                              | Downloads a completed billing or trust export payload for an authorized requester; unavailable or unfinished jobs do not expose export bodies through job metadata.                                                                                                                                |
...
| `POST /api/mail/outbox/:emailId/receipt-links`                                        | Creates a staff-only purpose-scoped receipt link for one authorized outbound email, stores only the token hash, and returns the raw token once.                                                                                                                                                    |
| `GET /api/portal/mail/receipts/:token`                                                | Public token-scoped receipt status with purpose, expiry, and acknowledgement timing only; it omits sessions, recipients, message bodies, token hashes, and email content.                                                                                                                          |
| `POST /api/portal/mail/receipts/:token/acknowledge`                                   | Public token-scoped acknowledgement that records receipt access without exposing sessions, recipients, message bodies, token hashes, or email content.                                                                                                                                             |
...
Reviewer decision records for those signals are append-only records under
`/api/contacts/*/review-decisions`. A decision must reference a current visible review signal and
may record duplicate-candidate, protected-party handling, or conflict-revalidation reviewer intent
only. The record shape intentionally excludes raw matched values, contact rewrite instructions,
merge targets, and conflict disposition changes; audit metadata stores counts and IDs only.

...
| `POST /api/portal/intake-forms/:token/items/:itemId/uploads`                          | Public token-scoped S3 upload intent for an intake upload item, including accepted MIME-type validation, signed checksum, byte-size, configured SSE-S3 headers, and scope metadata.                                                                                                                                                                                      |
| `POST /api/portal/intake-forms/:token/items/:itemId/documents/:documentId/complete`   | Public storage-verified checksum, byte-size, and configured SSE-S3 completion for an intake upload item document, returning generic upload receipt state rather than checksum/duplicate details.                                                                                                                                                                         |
...
| `POST /api/payments`                                                                  | Record a manual payment allocation with optional operator evidence; over-allocation is rejected and invoice balance/status is recalculated without live processor settlement or trust posting.                                                                                                                                                                           |
...
| `GET /api/billing/dashboard`                                                          | Billing dashboard payload for approved unbilled work, draft/submitted capture-review rows including non-billable draft time, timer-draft policy metadata, review-only expense category profiles, draft invoices, issued balances, payment request shells, manual payment evidence flags, period-lock summaries, and rate-rule summaries.                                 |
...
| `POST /api/inbound-email/provider-webhooks/mailgun/raw-mime`                          | Public signed Mailgun raw-MIME receiving endpoint that verifies the provider HMAC, stores the raw MIME object under the parser-owned `inbound-email/<firmId>/raw/` namespace, creates an idempotent `inbound_email` parser job, and returns only accepted/job status without storage keys, raw MIME, or signing material.                                                |
| `POST /api/inbound-email/parser-jobs/:jobId/retry`                                    | Owner-only confirmed manual retry for failed or dead-letter inbound parser lifecycle jobs, creating a new redacted parser job from the private raw-object pointer while leaving the source job intact and returning only redacted job summaries.                                                                                                                         |
| `POST /api/inbound-email/parser-jobs/:jobId/dead-letter`                              | Owner-only confirmed manual dead-letter transition for failed or stalled queued/active inbound parser lifecycle jobs, with current-status confirmation and redacted operator audit metadata only.                                                                                                                                                                        |
...
| `GET /api/inbound-email/messages?matterId=`                                           | Matter-scoped parsed inbound email messages, or firm-wide owner/auditor review queue; non-owner staff must supply an authorized matter scope.                                                                                                                                                                                                                            |
...
| `POST /api/document-processing/documents/:id/queue`                                   | Queues OCR for an authorized verified document only when the local OCR provider is enabled, object storage is configured, and the OCR worker queue is configured.                                                                                                                                                                                                        |
...
| `POST /api/portal/external-uploads/:token/intents`                                    | Public token-scoped S3 PUT upload intent for one external-upload link with required `fileSizeBytes` and configured SSE-S3 header when enabled; upload capacity is claimed before creating the pending document record.                                                                                                                                                   |
| `POST /api/portal/external-uploads/:token/documents/:documentId/complete`             | Public token-scoped storage-verified checksum, byte-size, scan-state, and configured SSE-S3 completion for a document upload intent, returning generic upload receipt state rather than checksum/duplicate details.                                                                                                                                                      |

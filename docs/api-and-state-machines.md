...
| `GET /api/documents/presign-upload`                                                   | S3 PUT upload intent, storage key, document intent record, required scan marker, required `fileSizeBytes` contract, and configured SSE-S3 header when enabled.                                                                                                                                                                                                           |
...
webhooks and automatic document promotion remain deferred. Concrete Postal, Tesseract, Whisper/FFmpeg,
and live Ollama/LM Studio adapters still require explicit setup, provider adapters, review states,

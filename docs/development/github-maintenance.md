...
- For release maintenance, collect changed scope, the `artifacts/release-local/<timestamp>/`
  ...
  SBOM output, and an artifact-secret scan result. Keep the artifact local unless the user explicitly
  asks to publish it, because command logs are validation evidence rather than a public release note.

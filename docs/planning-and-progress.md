...
| Candidate | OP-T36 | Email workflow closure           | Connect SMTP/outbox worker capability to API preview/send flows for signature, intake, share, upload, and future meeting notices while preserving Mailpit/Postal use.            | Run selector on worker/provider/API/database/docs paths; include worker/provider/API tests and typechecks, docs/policy checks, and `pnpm ci:local` for worker-to-API contract changes.                   |

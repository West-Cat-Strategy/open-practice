...
Status: Implemented with 2026-06-29 read-only reconciliation-readiness and structured reason-detail
follow-up proof recorded. Repository-wide format and policy checks have unrelated blockers
documented below.
...
- The 2026-06-29 reason-detail follow-up deepens those cues with structured safe per-row details
  for supported decisions that are still eligible or have drifted ineligible. The details are enum
  codes, readiness status values, and OP-authored labels over existing decision,
  duplicate/conflict, manual-payment, amount, invoice, and balance checks only.
...
- Reconciliation-readiness cues fail closed when the latest decision is not supported, duplicate or
  ...
  longer matches, or current invoice balance no longer covers the payment. Structured reason details
  explain the already-evaluated safe checks without adding free-form evidence or identifiers beyond
  existing candidate IDs and amounts. `/api/payments/:paymentId/reconcile` remains the only workflow
  that can create an effective allocation.
...
PASS pnpm --filter @open-practice/domain test -- billing.test.ts
  - Focused domain package run reported 33 files and 266 tests passing.
...
| -------------------------------------------------------------------------------------------------------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm verify:select -- --files <final path set>`                                                                                             | Pass    | Recommended architecture, API contract, format, docs, policy, domain/api/provider/worker/web tests, package typechecks/builds, and repo build for the final OP-T162 reason-detail path set.                     |
| `pnpm --filter @open-practice/domain test -- billing.test.ts`                                                                                | Pass    | Focused domain package run reported 33 files and 266 tests passing.                                                                                                                                             |
| `pnpm --dir apps/api exec vitest run src/routes/billing.test.ts`                                                                             | Pass    | Focused Billing API route suite reported 1 file and 32 tests passing with eligible and now-ineligible readiness detail assertions.                                                                              |
| `pnpm --dir apps/web exec vitest run app/dashboard/billing-section.test.tsx`                                                                 | Pass    | Focused Billing section render coverage reported 1 file and 1 test passing with readiness detail copy assertions.                                                                                               |
...
| `pnpm format:check`                                                                                                                          | Pass    | Final repo-wide Prettier check passed after scoped formatting of touched docs/domain-test files.                                                                                                                |
| `pnpm docs:check`                                                                                                                            | Pass    | Documentation link validation passed after docs formatting.                                                                                                                                                     |
| `pnpm policy:check`                                                                                                                          | Blocked | Blocked by unrelated central reference-index lock drift after secret scan, package manifest, supply-chain, toolchain, env, architecture, dead-code, migration parity, and migration lint passed.                |
| `pnpm --filter @open-practice/domain test`                                                                                                   | Pass    | Domain suite reported 33 files and 266 tests passing.                                                                                                                                                           |
...
| `pnpm --filter @open-practice/api test`                                                                                                      | Pass    | API suite reported 43 files and 623 tests passing.                                                                                                                                                              |
...
| `pnpm --filter @open-practice/web test`                                                                                                      | Pass    | Web suite reported 46 files and 243 tests passing.                                                                                                                                                              |
...
| `git diff --check -- <final path set>`                                                                                                       | Pass    | Scoped OP-T162 reason-detail whitespace check passed.                                                                                                                                                           |
| `pnpm proof:reconcile -- --proof docs/validation/OP-T162_DEPOSIT_MATCH_REVIEW_COMMAND_BOUNDARY_PROOF_2026-06-27.md --files <final path set>` | Pass    | Proof reconciliation passed for the 13-path OP-T162 reason-detail follow-up set and selected validation command set; `pnpm policy:check` remains explicitly blocked by unrelated central reference-index drift. |

## Notes

- The root `/Users/bryan/projects/open-practice` checkout had unrelated report-export edits, so this
  work was implemented in clean sibling worktree
  `/Users/bryan/projects/open-practice-op-t162-readiness-reasons` on branch
  `feat/op-t162-readiness-reason-details-20260629`.

...
The 2026-07-01 Docker daemon preflight blocker proof is recorded in
[OP_DOCKER_DAEMON_PREFLIGHT_PROOF_2026-07-01.md](OP_DOCKER_DAEMON_PREFLIGHT_PROOF_2026-07-01.md).
It covers the local-only daemon reachability preflight shared by Docker storage/app-smoke,
residual-watch, and image-scan wrappers, including deterministic `docker_daemon_unavailable`
blocker artifacts, selector-driven validation, and preserved Compose contracts, image pins, ports,
credentials, runtime APIs, schemas, dependencies, and application behavior.
...
It covers descriptive fixture coverage for refund/chargeback review, trust-transfer review, staff
report export, audit export, billing export, and jurisdictional trust export authorization posture,
...
trust-transfer approve/reject/link, and trust posting approve/reject commands, including
missing/stale/fresh route tests, selector-driven validation, and preserved role checks, matter
scope, self-approval denial, idempotency, no-overdraft and balance checks, audit behavior, provider
posture, and financial command semantics.
...
It covers the additive read-only staff report and Billing dashboard projection over existing
billing period locks, time entries, expense entries, invoices, and visible matters, including
lock/status/matter/source-type counts, sorted safe IDs only, selector-driven validation, and
preserved no unlock, override, bypass, mutation, enforcement, migration, provider, settlement,
...
The 2026-06-30 calendar aging follow-up task proof is recorded in
[OP_CALENDAR_AGING_FOLLOW_UP_TASK_PROOF_2026-06-30.md](OP_CALENDAR_AGING_FOLLOW_UP_TASK_PROOF_2026-06-30.md).
It covers the staff-only task command for the latest eligible `follow_up_required`
calendar-aging decision across appointment booking tentative holds and calendar scheduling
requests, including matter-scoped authorization, duplicate-source prevention, fixed redacted task
copy, Calendar dashboard action wiring, selector-driven validation, and preserved no hold
confirmation, request expiry, provider sync, reminder queueing, public rooms, native
media/chat/recording, or matter creation.

The 2026-07-01 legal-clinic cadence task command proof is recorded in
[OP_LEGAL_CLINIC_CADENCE_TASK_PROOF_2026-07-01.md](OP_LEGAL_CLINIC_CADENCE_TASK_PROOF_2026-07-01.md).
It covers the staff-only `POST /api/tasks/legal-clinic-cadence-follow-up` command over existing
legal-clinic cadence signals, including matter-scoped authorization, duplicate-source suppression
across completed/archived tasks, fixed redacted task copy, safe audit metadata, dashboard explicit
action wiring, selector-driven validation, and preserved no automatic task creation, provider sync,
client-visible workflow, or cadence/profile/program mutation.

The 2026-06-29 report export download audit events proof is recorded in
[OP_REPORT_EXPORT_DOWNLOAD_AUDIT_EVENTS_PROOF_2026-06-29.md](OP_REPORT_EXPORT_DOWNLOAD_AUDIT_EVENTS_PROOF_2026-06-29.md).
It covers safe downloaded audit events for completed audit, billing, staff report, and
jurisdictional trust export jobs, including job/profile/scope/count metadata, explicit no-storage
flags, billing export taxonomy de-duplication, selector-driven validation, and preserved no custom
SQL, BI embeds, scheduled execution/email, raw body storage, private audit payload exposure, export
serialization/profile changes, payment mutation, trust posting, or certification claims.
...
The 2026-07-01 inbound parser replay inventory proof is recorded in
[OP_INBOUND_PARSER_REPLAY_INVENTORY_PROOF_2026-07-01.md](OP_INBOUND_PARSER_REPLAY_INVENTORY_PROOF_2026-07-01.md).
It covers the owner-admin-only, metadata-only inventory over failed and dead-letter inbound parser
jobs, including safe job IDs, provider-family labels, allowlisted failure stages, age/attempt
counts, fixed no-raw-MIME/no-object-key/no-provider-payload/no-mailbox-secret/no-document-promotion/
no-matter-creation flags, communications dashboard rendering, selector-driven validation, and
preserved no replay enqueue, object copy/read, storage-key exposure, provider execution, document
promotion, matter creation, schema/migration, dependency, worker, or provider-adapter change.
...
and financial export field profiles, including bounded field-key samples, profile-purpose/scope
differences, disabled-safeguard flags, selector-driven validation, and preserved no custom SQL, BI
embeds, scheduled execution/email, raw report-body storage, payment processor exposure, payment
creation/allocation, invoice mutation, trust posting, or certification claims.

The 2026-07-01 expense category accounting export profile preview proof is recorded in
[OP_EXPENSE_CATEGORY_ACCOUNTING_EXPORT_PROFILE_PREVIEW_PROOF_2026-07-01.md](OP_EXPENSE_CATEGORY_ACCOUNTING_EXPORT_PROFILE_PREVIEW_PROOF_2026-07-01.md).
It covers the additive read-only Reports workspace preview that maps existing firm expense category
codes to an OP-authored accounting export profile summary, including bounded mapping rows, category
counts, OP-authored review buckets/cues, selector-driven validation, and preserved no external
accounting provider, export serialization change, invoice recalculation, payment mutation, trust
posting, or certified-accounting claim.

The 2026-06-29 calendar aging review decisions proof is recorded in
[OP_CALENDAR_AGING_REVIEW_DECISIONS_PROOF_2026-06-29.md](OP_CALENDAR_AGING_REVIEW_DECISIONS_PROOF_2026-06-29.md).
It covers enum-only staff triage decisions for aging/stale appointment booking tentative holds and
calendar scheduling requests, including row-level persistence, staff-only API routes, safe audit
metadata, dashboard decision controls, selector-driven validation, and preserved no auto-confirm,
auto-expiry, provider sync, public room, native media, chat, recording, reminder queueing, task
creation, or matter creation.

The 2026-06-29 refund/chargeback review decisions proof is recorded in
[OP_REFUND_CHARGEBACK_REVIEW_DECISIONS_PROOF_2026-06-29.md](OP_REFUND_CHARGEBACK_REVIEW_DECISIONS_PROOF_2026-06-29.md).
It covers the staff-only, provider-neutral enum decision records over existing payment import
refund/chargeback cues, including strict body validation, idempotent replay/conflict behavior,
reason/category guardrails, safe audit metadata, Billing dashboard decision counts/latest decision
...
The 2026-06-29 inbound communications aggregation efficiency closeout proof, refreshed on
2026-07-01, is recorded in
...
attachment reads by visible `inboundMessageIds` and conversation message/notification reads by
visible `threadIds` for `/api/communications/inbox`, while preserving response shapes,
authorization, redaction, provider, queue/retry, schema, migration, and synthetic-data boundaries.

The 2026-06-29 Trust Controls policy preview matrix proof is recorded in
[OP_TRUST_CONTROLS_POLICY_PREVIEW_MATRIX_PROOF_2026-06-29.md](OP_TRUST_CONTROLS_POLICY_PREVIEW_MATRIX_PROOF_2026-06-29.md).
It covers the additive read-only `makerCheckerReadiness.policyPreviewMatrix` on
`GET /api/ledger/controls`, the Trust Controls dashboard matrix, safe matter-scoped filtering, and
the preserved no-policy-enabled, direct-posting-unchanged, no-approval/rejection-mutation,
no-auto-posting, no-settlement, no-auto-match, no-bank-feed-connection, and
...
The 2026-06-28/2026-06-29 mainline consolidation proof is recorded in
[OP_MAINLINE_MERGE_PUSH_PRUNE_PROOF_2026-06-28.md](OP_MAINLINE_MERGE_PUSH_PRUNE_PROOF_2026-06-28.md).
It covers the validated `integrate/mainline-consolidation-20260629` closeout of legal-research
action descriptors, document disposition metadata, OP-T158 email template publish/version history,
Trust Controls maker-checker readiness, and the proof-only inbound communications aggregation
closeout.
...
It covers the metadata-only runtime decision command for existing conversion-review artifacts,
including `PATCH /api/document-processing/documents/:documentId/conversion-review/review`,
matter-scoped `document_processing:read` plus `legal_research:approve` authorization, reviewed/
rejected terminal posture, no-op same-decision replay, opposite-terminal and not-ready conflicts,
safe response/audit metadata, selector-driven validation, and preserved no raw client text,
converted Markdown, annotations, chunks, embeddings, prompts, provider payloads, storage keys,
object bodies, generated summaries, provider activation, new jobs, or downstream source-record
mutation boundaries.

The 2026-06-29 provider document conversion review cues proof is recorded in
[OP_PROVIDER_DOCUMENT_CONVERSION_REVIEW_CUES_PROOF_2026-06-29.md](OP_PROVIDER_DOCUMENT_CONVERSION_REVIEW_CUES_PROOF_2026-06-29.md).
It covers read-only latest-decision and bounded history cues derived only from terminal
`document_analysis_status` artifact review fields, including document-processing workbench, queue
replay, review-command responses, Documents copy, Research projection copy, focused API/web
assertions, selector-driven validation, and preserved no raw text, Markdown, annotations, chunks,
embeddings, prompts, provider payloads, storage keys, object bodies, generated summaries, new jobs,
schema/migration/dependency change, provider activation, or downstream source-record mutation.

The 2026-06-29 provider document conversion semantic-review readiness proof is recorded in
[OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_READINESS_PROOF_2026-06-29.md](OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_READINESS_PROOF_2026-06-29.md).
It covers additive `conversionReview.semanticReviewReadiness` packets over existing summaries for
same-matter ready/reviewed conversion-review artifacts, blocked posture for not-ready states,
focused API assertions, selector-driven validation, and preserved no providers, schema, queues,
storage write, semantic-review execution, raw text, Markdown, annotations, chunks, embeddings,
prompts, provider payloads, storage keys, object bodies, generated summaries, or downstream
mutation.

The 2026-06-29 provider document conversion semantic-review checkpoints proof is recorded in
[OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_CHECKPOINTS_PROOF_2026-06-29.md](OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_CHECKPOINTS_PROOF_2026-06-29.md).
It covers the explicit staff command that creates or replays reviewer-owned metadata-only
`review_checkpoint` artifacts for existing same-matter ready/reviewed conversion-review artifacts,
bounded checkpoint count/latest-cue projection in `semanticReviewReadiness`, Documents dashboard
action wiring, focused domain/database/API/web validation, selector-driven validation, and preserved
no providers, queues, object-body or text-extraction reads, raw text, Markdown, annotations, chunks,
embeddings, prompts, provider payloads, storage keys, generated summaries, downstream mutation,
schema, migration, dependency, or automatic checkpoint creation.

The 2026-07-01 provider document conversion semantic-review preflight packet proof is recorded in
[OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_PREFLIGHT_PACKET_PROOF_2026-07-01.md](OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_PREFLIGHT_PACKET_PROOF_2026-07-01.md).
It covers the nested metadata-only `conversionReview.semanticReviewReadiness.preflightPacket` over
existing same-matter ready/reviewed conversion-review artifacts and semantic checkpoint cues,
including focused domain/API/web assertions, selector-driven validation, and preserved no new
route, schema, migration, dependency, provider, queue, worker, object-body read, text-extraction
read, raw OCR text, Markdown, annotations, chunks, embeddings, prompts, storage keys, provider
payloads, generated summaries, storage write, or downstream mutation.

The 2026-06-29 legal-research artifact review action-descriptor proof is recorded in
[OP_LEGAL_RESEARCH_ARTIFACT_REVIEW_ACTION_DESCRIPTORS_PROOF_2026-06-29.md](OP_LEGAL_RESEARCH_ARTIFACT_REVIEW_ACTION_DESCRIPTORS_PROOF_2026-06-29.md).
It covers the behavior-preserving descriptor follow-up for existing Research workspace artifact
`Review`/`Reject` controls, including domain-owned labels, busy/disabled reasons, stable action
keys, accessible status text, focused domain/web validation, and preserved legal-research and
document-processing route/API behavior, authorization, persistence, provider posture, and
review-only boundaries.

The 2026-06-27 OP-T162 deposit-match review command boundary proof is recorded in
[OP-T162_DEPOSIT_MATCH_REVIEW_COMMAND_BOUNDARY_PROOF_2026-06-27.md](OP-T162_DEPOSIT_MATCH_REVIEW_COMMAND_BOUNDARY_PROOF_2026-06-27.md).
It covers the staff-only, provider-neutral, append-only deposit-match reviewer decision command over
existing payment import review records, including idempotent replay, same-matter candidate
validation, candidate-supported guardrails, Billing dashboard latest-decision cues, read-only manual
reconcile readiness cues, safe audit metadata, selector-driven validation, and preserved no live
settlement, provider commands, raw provider payload retention, invoice-balance mutation,
reconciliation automation, refund/chargeback workflow, client notifications, trust transfers, or
trust posting.

The 2026-06-28 refund/chargeback review cue proof is recorded in
[OP_REFUND_CHARGEBACK_REVIEW_CUES_PROOF_2026-06-28.md](OP_REFUND_CHARGEBACK_REVIEW_CUES_PROOF_2026-06-28.md).
It covers provider-neutral, metadata-only refund/chargeback review cues derived from existing
payment import review records, including Billing dashboard counts, optional row cue metadata, safe
payment-import audit metadata, selector-driven validation, and preserved no provider calls, dispute
packet storage, invoice mutation, ledger reversal, client notification, trust transfer, or trust
posting.

The 2026-06-28 calendar aging review cue proof is recorded in
[OP_CALENDAR_AGING_REVIEW_CUES_PROOF_2026-06-28.md](OP_CALENDAR_AGING_REVIEW_CUES_PROOF_2026-06-28.md).
It covers review-only fresh/aging/stale cues for open appointment booking tentative holds and open
calendar scheduling requests, including staff-only API summaries, Calendar/dashboard row copy and
counts, selector-driven validation, and preserved no automatic final confirmation, auto-expiry,
provider sync, public room URLs, native media, chat, recordings, or matter creation.
...
The 2026-06-29 document disposition metadata proof is recorded in
[OP_DOCUMENT_DISPOSITION_METADATA_PROOF_2026-06-29.md](OP_DOCUMENT_DISPOSITION_METADATA_PROOF_2026-06-29.md).
It covers the read-only `dispositionMetadata` projection inside the document-processing workbench
`retentionHoldReview` packet, including candidate state, blocker counts, source cue counts, optional
review schedule fields, Documents dashboard rendering, selector-driven validation, and preserved no
new route, command, schema, migration, audit write, object deletion, retention-deadline enforcement,
legal-hold release command, raw payload retention, or compliance claim.

The 2026-06-29 document disposition schedule profile proof is recorded in
[OP_DOCUMENT_DISPOSITION_SCHEDULE_PROFILE_PROOF_2026-06-29.md](OP_DOCUMENT_DISPOSITION_SCHEDULE_PROFILE_PROOF_2026-06-29.md).
It covers the bounded firm-settings default profile, staff-only read and `firm:update` write routes,
safe audit metadata, workbench projection into
`retentionHoldReview.dispositionMetadata.scheduleProfile`, Documents dashboard context rendering,
selector-driven validation, and preserved no object deletion, deadline enforcement, legal-hold
override, raw/export body retention, public/client controls, or compliance claim.

The 2026-06-30 document disposition reviewer packet proof is recorded in
[OP_DOCUMENT_DISPOSITION_REVIEWER_PACKET_PROOF_2026-06-30.md](OP_DOCUMENT_DISPOSITION_REVIEWER_PACKET_PROOF_2026-06-30.md).
It covers the staff-only, matter-scoped document-processing command for enum-only disposition
reviewer packet decisions over `retentionHoldReview.dispositionMetadata`, safe audit metadata,
Documents dashboard rendering, selector-driven validation, and preserved no object deletion,
deadline enforcement, legal-hold release/override, raw OCR/provider/export body retention, public
client controls, or compliance claim.
...
The 2026-06-27 matter lifecycle reopen boundary proof is recorded in
[OP_MATTER_LIFECYCLE_REOPEN_BOUNDARY_PROOF_2026-06-27.md](OP_MATTER_LIFECYCLE_REOPEN_BOUNDARY_PROOF_2026-06-27.md).
It covers the narrow status-only closed/archived `reopen` boundary for `POST
/api/matters/:matterId/lifecycle-commands`, latest-ready readiness gating, safe audit metadata with
`retentionChanged: false`, and preserved portal/task/assignment/billing/trust/retention/cleanup/
source-record side effects.
...
The 2026-06-29 OP-T158 email template publish/version-history proof is recorded in
[OP-T158_EMAIL_TEMPLATE_PUBLISH_HISTORY_PROOF_2026-06-29.md](OP-T158_EMAIL_TEMPLATE_PUBLISH_HISTORY_PROOF_2026-06-29.md).
It covers immutable published versions beside mutable drafts, firm-scoped publish/history routes,
compact staff dashboard publish/history controls, redacted publish audit metadata, route
authorization coverage, selector-driven validation, and preserved render-only preview, no-campaign,
no-bulk-send, no-subscription, no-provider-delivery, no-queue, and no-send-job boundaries.

The 2026-06-29 OP-T158 email template compare proof is recorded in
[OP-T158_EMAIL_TEMPLATE_COMPARE_PROOF_2026-06-29.md](OP-T158_EMAIL_TEMPLATE_COMPARE_PROOF_2026-06-29.md).
It covers the read-only staff dashboard compare surface between persisted saved drafts and selected
immutable published versions, provider-neutral domain comparison helpers, focused domain/web
validation, and preserved render-only preview, no-campaign, no-bulk-send, no-subscription,
no-provider-delivery, no-queue, no-send-job, no-route, no-migration, and no-persistence-change
boundaries.

The 2026-06-30 email template reviewed outbound preview handoff proof is recorded in
[OP_EMAIL_TEMPLATE_REVIEWED_OUTBOUND_PREVIEW_HANDOFF_PROOF_2026-06-30.md](OP_EMAIL_TEMPLATE_REVIEWED_OUTBOUND_PREVIEW_HANDOFF_PROOF_2026-06-30.md).
It covers the staff-only handoff from immutable published versions into single-recipient
matter/contact reviewed previews, safe preview/audit metadata, selector-driven validation, and
preserved no-send, no-outbox, no-provider, no-queue, no-campaign, no-bulk-send, no-subscription, and
no-permission-widening boundaries.
...
| Artifact                                                                                                                                                                     | Type                 | Use                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [OP_FINANCIAL_AUTHORIZATION_FIXTURE_CATALOGUE_PROOF_2026-06-29.md](OP_FINANCIAL_AUTHORIZATION_FIXTURE_CATALOGUE_PROOF_2026-06-29.md)                                         | Row-local proof note | Proof for the descriptive authorization fixture catalogue extension across refund/chargeback review, trust-transfer review, staff report export, audit export, billing export, and jurisdictional trust export surfaces, covering existing staff/auditor/bookkeeper/client-external list visibility and command denial behavior without adding roles, policies, OpenFGA runtime, route changes, or portal exposure.                                                                                                                                                                                                                                                                               |
| [OP_FINANCIAL_COMMAND_FRESH_AUTH_PROOF_2026-06-29.md](OP_FINANCIAL_COMMAND_FRESH_AUTH_PROOF_2026-06-29.md)                                                                   | Row-local proof note | Proof for reusing existing fresh-session step-up on high-risk financial command mutations, covering payment reconcile, trust-transfer approve/reject/link, trust posting approve/reject, focused missing/stale/fresh route tests, selector-driven validation, and preserved role checks, matter scope, self-approval denial, idempotency, no-overdraft and balance checks, audit behavior, provider posture, and financial command semantics.                                                                                                                                                                                                                                                     |
| [OP_BILLING_PERIOD_LOCK_IMPACT_PROJECTION_PROOF_2026-06-29.md](OP_BILLING_PERIOD_LOCK_IMPACT_PROJECTION_PROOF_2026-06-29.md)                                                 | Row-local proof note | Proof for the additive read-only billing period lock impact projection over existing locks, time entries, expense entries, invoices, and authorized visible matters, covering lock/status/matter/source-type grouping, sorted safe IDs, Reports and Billing dashboard rendering, selector-driven validation, and preserved no unlock/override/bypass controls, source-record mutation, lock-enforcement change, migration, provider/settlement/trust-posting behavior, raw report-body storage, narratives, descriptions, memos, client identifiers, or payment details.                                                                                                                          |
| [OP_EMAIL_TEMPLATE_REVIEWED_OUTBOUND_PREVIEW_HANDOFF_PROOF_2026-06-30.md](OP_EMAIL_TEMPLATE_REVIEWED_OUTBOUND_PREVIEW_HANDOFF_PROOF_2026-06-30.md)                           | Row-local proof note | Proof for the staff-only provider-neutral handoff from immutable published email template versions into single-recipient reviewed outbound previews, covering matter/contact authorization, email-method eligibility, strict body guardrails, safe stored preview/audit metadata, dashboard selector/recent-list rendering, selector-driven validation, and preserved no send, outbox, provider call, provider settings inspection, queue/job, campaign, bulk-send, subscription, automation, raw template-body audit metadata, or permission widening.                                                                                                                                           |
| [OP_CALENDAR_AGING_FOLLOW_UP_TASK_PROOF_2026-06-30.md](OP_CALENDAR_AGING_FOLLOW_UP_TASK_PROOF_2026-06-30.md)                                                                 | Row-local proof note | Proof for the staff-only calendar aging follow-up task command, covering latest eligible `follow_up_required` selection across appointment booking tentative holds and calendar scheduling requests, matter-scoped `task:create` authorization, duplicate-source prevention, fixed redacted task copy, Calendar dashboard action/status refresh, selector-driven validation, and preserved no hold confirmation, request expiry, provider sync, reminder queueing, public rooms, native media/chat/recording, or matter creation.                                                                                                                                                                 |
| [OP_LEGAL_CLINIC_CADENCE_TASK_PROOF_2026-07-01.md](OP_LEGAL_CLINIC_CADENCE_TASK_PROOF_2026-07-01.md)                                                                         | Row-local proof note | Proof for the staff-only legal-clinic cadence task command, covering latest-eligible cadence selection, matter-scoped authorization, duplicate `operational_view` source suppression including completed/archived matches, fixed redacted internal task copy, safe `task.created` audit provenance, dedicated Tasks dashboard action wiring, selector-driven validation, and preserved no automatic task creation, provider sync, client-visible workflow, or cadence/profile/program mutation.                                                                                                                                                                                                   |
| [OP_MAINLINE_MERGE_PUSH_PRUNE_PROOF_2026-06-29.md](OP_MAINLINE_MERGE_PUSH_PRUNE_PROOF_2026-06-29.md)                                                                         | Mainline proof       | Proof for the 2026-06-29 all-active-lane closeout through `integrate/mainline-consolidation-20260629-active-lanes`, covering report export profile alignment, scheduling aging decisions, document disposition schedule profile, email template compare, OP-T162 readiness reasons, payment-import authorization fixtures, refund/chargeback decisions, semantic readiness, Trust Controls policy preview matrix, deterministic migration `0075`/`0076`/`0077` reconciliation, selector-driven validation, final `main` publication, and clean merged worktree/branch pruning.                                                                                                                    |
| [OP_REFUND_CHARGEBACK_REVIEW_DECISIONS_PROOF_2026-06-29.md](OP_REFUND_CHARGEBACK_REVIEW_DECISIONS_PROOF_2026-06-29.md)                                                       | Row-local proof note | Proof for the staff-only refund/chargeback review decision slice over existing payment import exception cues, covering enum-only decisions/reasons, reason/category guardrails, idempotent replay and conflict behavior, strict raw/artifact payload rejection, safe audit metadata, Billing dashboard decision counts and latest-decision visibility, selector-driven validation, and preserved no provider calls, refund/dispute artifacts, invoice mutation, ledger reversal, client notification, trust posting, or funds movement.                                                                                                                                                           |
| [OP_MAINLINE_MERGE_PUSH_PRUNE_PROOF_2026-06-28.md](OP_MAINLINE_MERGE_PUSH_PRUNE_PROOF_2026-06-28.md)                                                                         | Mainline proof       | Proof for the 2026-06-28/2026-06-29 active-lane consolidation through `integrate/mainline-consolidation-20260629`, covering legal-research action descriptors, document disposition metadata, OP-T158 email template publish/version history with migration `0074`, Trust Controls maker-checker readiness, inbound communications aggregation proof closeout, selector-driven validation, and the known unrelated OSS reference-lock policy drift.                                                                                                                                                                                                                                               |
| [OP_INBOUND_COMMUNICATIONS_AGGREGATION_EFFICIENCY_PROOF_2026-06-29.md](OP_INBOUND_COMMUNICATIONS_AGGREGATION_EFFICIENCY_PROOF_2026-06-29.md)                                 | Maintenance proof    | Proof-only closeout, refreshed on 2026-07-01, for the inbound communications aggregation efficiency slice, covering existing `/api/communications/inbox` child-row batching by authorized `inboundMessageIds` and `threadIds`, focused repository/API validation, and preserved response-shape, authorization, redaction, provider, queue/retry, schema, migration, and synthetic-data boundaries.                                                                                                                                                                                                                                                                                                |
| [OP_INBOUND_PARSER_REPLAY_INVENTORY_PROOF_2026-07-01.md](OP_INBOUND_PARSER_REPLAY_INVENTORY_PROOF_2026-07-01.md)                                                             | Row-local proof note | Proof for the owner-admin-only, metadata-only inbound parser replay inventory over failed and dead-letter parser jobs, covering safe job IDs, provider-family labels, allowlisted failure stages, age/attempt counts, fixed no-private-payload flags, Communications dashboard rendering, selector-driven validation, and preserved no replay enqueue, object copy/read, storage-key exposure, provider execution, document promotion, matter creation, schema/migration, dependency, worker, or provider-adapter change.                                                                                                                                                                         |
| [OP_CALENDAR_AGING_REVIEW_DECISIONS_PROOF_2026-06-29.md](OP_CALENDAR_AGING_REVIEW_DECISIONS_PROOF_2026-06-29.md)                                                             | Row-local proof note | Proof for enum-only staff aging-review decisions over stale/aging appointment booking tentative holds and calendar scheduling requests, covering nullable latest-decision persistence, staff-only routes, safe audit metadata, dashboard triage controls, selector-driven validation, and preserved no lifecycle mutation, provider sync, public room, native media, chat, recording, reminder queueing, task creation, or matter creation.                                                                                                                                                                                                                                                       |
| [OP_CALENDAR_AGING_REVIEW_CUES_PROOF_2026-06-28.md](OP_CALENDAR_AGING_REVIEW_CUES_PROOF_2026-06-28.md)                                                                       | Row-local proof note | Proof for review-only fresh/aging/stale cues over open appointment booking tentative holds and open calendar scheduling requests, covering domain helper thresholds, staff-only API summaries, Calendar/dashboard row copy and counts, selector-driven validation, and preserved no automatic final confirmation, auto-expiry, provider sync, public room URLs, native media, chat, recordings, or matter creation.                                                                                                                                                                                                                                                                               |
| [OP_REFUND_CHARGEBACK_REVIEW_CUES_PROOF_2026-06-28.md](OP_REFUND_CHARGEBACK_REVIEW_CUES_PROOF_2026-06-28.md)                                                                 | Row-local proof note | Proof for the metadata-only refund/chargeback review cue surface over existing payment import review records, covering safe `refund_observed` and `chargeback_observed` payment statuses, Billing dashboard cue counts and row metadata, allowlisted audit metadata, selector-driven validation, and preserved no provider calls, dispute packets, invoice mutation, ledger reversal, client notification, trust transfer, or trust posting.                                                                                                                                                                                                                                                      |
| [OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_CHECKPOINTS_PROOF_2026-06-29.md](OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_CHECKPOINTS_PROOF_2026-06-29.md)           | Row-local proof note | Proof for explicit metadata-only semantic review checkpoint records over existing same-matter ready/reviewed document conversion-review artifacts, covering the strict-empty-body staff command, idempotent `review_checkpoint` artifact creation/replay, bounded checkpoint count/latest-cue projection, Documents dashboard action, focused domain/database/API/web validation, selector-driven validation, and preserved no providers, queues, object-body or text-extraction reads, raw text, Markdown, annotations, chunks, embeddings, prompts, provider payloads, storage keys, generated summaries, downstream mutation, schema, migration, dependency, or automatic checkpoint creation. |
| [OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_PREFLIGHT_PACKET_PROOF_2026-07-01.md](OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_PREFLIGHT_PACKET_PROOF_2026-07-01.md) | Row-local proof note | Proof for nested metadata-only semantic-review preflight packets over existing same-matter ready/reviewed document conversion-review artifacts and checkpoint cues, covering focused domain/API/web assertions, selector-driven validation, and preserved no new route, schema, migration, dependency, provider, queue, worker, object-body read, text-extraction read, raw OCR text, Markdown, annotations, chunks, embeddings, prompts, storage keys, provider payloads, generated summaries, storage write, or downstream mutation.                                                                                                                                                            |
| [OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_READINESS_PROOF_2026-06-29.md](OP_PROVIDER_DOCUMENT_CONVERSION_SEMANTIC_REVIEW_READINESS_PROOF_2026-06-29.md)               | Row-local proof note | Proof for additive metadata-only `conversionReview.semanticReviewReadiness` packets over existing document conversion-review summaries, covering ready posture for same-matter `ready_for_review` or `reviewed` artifacts, blocked posture for rejected/queued/failed/not-requested/missing/draft states, focused API assertions, selector-driven validation, and preserved no providers, schemas, queues, storage writes, semantic-review execution, raw text, Markdown, annotations, chunks, embeddings, prompts, provider payloads, storage keys, object bodies, generated summaries, or downstream mutation.                                                                                  |
| [OP_PROVIDER_DOCUMENT_CONVERSION_REVIEW_CUES_PROOF_2026-06-29.md](OP_PROVIDER_DOCUMENT_CONVERSION_REVIEW_CUES_PROOF_2026-06-29.md)                                           | Row-local proof note | Proof for read-only latest-decision/history cues over existing terminal document conversion review artifacts, covering additive `conversionReview.latestDecision` and bounded newest-first `decisionHistory`, workbench/queue/review responses, Documents and Research metadata-only projection copy, selector-driven validation, and preserved no raw text, Markdown, annotations, chunks, embeddings, prompts, provider payloads, storage keys, object bodies, generated summaries, new jobs, schema/migration/dependency changes, provider activation, or downstream source-record mutation.                                                                                                   |
| [OP_PROVIDER_DOCUMENT_CONVERSION_REVIEW_DECISION_PROOF_2026-06-27.md](OP_PROVIDER_DOCUMENT_CONVERSION_REVIEW_DECISION_PROOF_2026-06-27.md)                                   | Row-local proof note | Proof for the metadata-only provider document conversion review decision slice, covering the explicit `reviewed`/`rejected` command over existing ready `document_analysis_status` artifacts, multi-guard authorization, terminal no-op/conflict behavior, safe summary/audit metadata, selector-driven validation, and preserved no raw text, retained Markdown, annotations, chunks, embeddings, prompts, provider payloads, storage keys, object bodies, generated summaries, provider activation, new jobs, schema/migration/dependency change, or downstream source-record mutation boundaries.                                                                                              |
| [OP_LEGAL_RESEARCH_CITATION_PACKET_READINESS_PROOF_2026-06-29.md](OP_LEGAL_RESEARCH_CITATION_PACKET_READINESS_PROOF_2026-06-29.md)                                           | Row-local proof note | Proof for additive, derived `citationPacketReadiness` packets on the staff-only Legal Research workspace API and Research dashboard, covering source/context counts, ready/open-checkpoint artifact IDs, reserved provider-job posture, selector-driven validation, and preserved no provider execution, authority scraping, source-text/prompt/provider-evidence storage, citation verification, legal advice, downstream mutation, schema/migration, route, worker, or permission widening.                                                                                                                                                                                                     |
| [OP_LEGAL_RESEARCH_ARTIFACT_REVIEW_ACTION_DESCRIPTORS_PROOF_2026-06-29.md](OP_LEGAL_RESEARCH_ARTIFACT_REVIEW_ACTION_DESCRIPTORS_PROOF_2026-06-29.md)                         | Row-local proof note | Proof for the behavior-preserving Research workspace legal-research artifact review action-descriptor follow-up, covering domain-owned `Review`/`Reject` labels, stable action keys, busy/disabled reasons, accessible status text, focused domain/web validation, and preserved legal-research/document-processing route/API behavior, authorization, persistence, provider posture, and review-only boundaries.                                                                                                                                                                                                                                                                                 |
| [OP_MAINLINE_MERGE_PUSH_PRUNE_PROOF_2026-06-30.md](OP_MAINLINE_MERGE_PUSH_PRUNE_PROOF_2026-06-30.md)                                                                         | Mainline proof       | Blocked proof for the 2026-06-30 all-worktree integration through `integrate/open-practice-all-worktrees-20260630`, covering lane commits, additive conflict resolution, deterministic `0078`/`0079` migration reconciliation, selector-driven validation, focused validation fixes, current security-review blockers, unchanged stash count, and the no-push/no-prune state because required validation did not pass.                                                                                                                                                                                                                                                                            |
| [OP-T162_DEPOSIT_MATCH_REVIEW_COMMAND_BOUNDARY_PROOF_2026-06-27.md](OP-T162_DEPOSIT_MATCH_REVIEW_COMMAND_BOUNDARY_PROOF_2026-06-27.md)                                       | Row-local proof note | Proof for the OP-T162 deposit-match review command boundary, covering append-only staff decisions over normalized payment import review records, idempotent replay, same-matter candidate validation, candidate-supported guardrails, Billing dashboard latest-decision cues, read-only manual reconcile readiness cues with structured safe reason details, safe audit metadata, selector-driven validation, and preserved no live settlement, provider commands, raw provider payload retention, invoice-balance mutation, reconciliation automation, refund/chargeback workflow, client notifications, trust transfers, or trust posting.                                                      |
...
| [OP_VERIFY_RUN_PLAN_MODE_PROOF_2026-06-30.md](OP_VERIFY_RUN_PLAN_MODE_PROOF_2026-06-30.md)                                                                                   | Maintenance proof    | Proof for print-only `pnpm verify:run -- --plan`, covering deterministic selector reuse, no command execution, no `.tmp/validation-runs` artifact writes, parser rejection of artifact-writing runner options, focused script tests, selector-driven validation, and preserved normal `verify:run`, dry-run, and `verify:select` semantics.                                                                                                                                                                                                                                                                                                                                                       |
| [OP_DOCKER_DAEMON_PREFLIGHT_PROOF_2026-07-01.md](OP_DOCKER_DAEMON_PREFLIGHT_PROOF_2026-07-01.md)                                                                             | Maintenance proof    | Proof for deterministic Docker daemon local-environment blockers in residual-watch, app-smoke/storage preflight, and image-scan wrappers, covering focused script tests, selector-driven validation, blocked local Docker wrapper evidence, and preserved Compose contracts, image pins, ports, credentials, runtime APIs, schemas, dependencies, app behavior, synthetic-only proof, and clean-room posture.                                                                                                                                                                                                                                                                                     |
...
| [OP_POLICY_SECRET_SCAN_SKIPPED_TRACKED_FILES_PROOF_2026-06-29.md](OP_POLICY_SECRET_SCAN_SKIPPED_TRACKED_FILES_PROOF_2026-06-29.md)                                           | Maintenance proof    | Proof for tightening `policy:check` so tracked files skipped by the tracked-secret scanner fail the gate, covering focused scanner tests, selector-driven validation, and preserved no secret-regex broadening, matched-value serialization, default ignored-evidence scanning, `.gitleaksignore` weakening, or synthetic false-positive policy change.                                                                                                                                                                                                                                                                                                                                           |
...
| [OP-T158_EMAIL_TEMPLATE_COMPARE_PROOF_2026-06-29.md](OP-T158_EMAIL_TEMPLATE_COMPARE_PROOF_2026-06-29.md)                                                                     | Row-local proof note | Proof for the read-only staff email template compare surface, covering provider-neutral saved-draft versus immutable published-version comparison helpers, dashboard default-newest version selection, escaped text rendering for body values, selector-driven validation, and preserved render-only preview, no-campaign, no-bulk-send, no-subscription, no-provider-delivery, no-queue, no-send-job, no-route, no-migration, and no-persistence-change boundaries.                                                                                                                                                                                                                              |
| [OP-T158_EMAIL_TEMPLATE_PUBLISH_HISTORY_PROOF_2026-06-29.md](OP-T158_EMAIL_TEMPLATE_PUBLISH_HISTORY_PROOF_2026-06-29.md)                                                     | Row-local proof note | Proof for immutable email template published versions beside mutable provider-neutral drafts, covering firm-scoped publish/history routes, staff dashboard publish/history controls, redacted publish audit metadata, route authorization coverage, selector-driven validation, and preserved render-only preview, no-campaign, no-bulk-send, no-subscription, no-provider-delivery, no-queue, and no-send-job boundaries.                                                                                                                                                                                                                                                                        |
...
| [OP_DOCUMENT_DISPOSITION_METADATA_PROOF_2026-06-29.md](OP_DOCUMENT_DISPOSITION_METADATA_PROOF_2026-06-29.md)                                                                 | Row-local proof note | Proof for the read-only document disposition metadata slice, covering derived `retentionHoldReview.dispositionMetadata` candidate state, blocker counts, source cue counts, optional review schedule fields, Documents dashboard rendering, selector-driven validation, and preserved no new route/command, schema, migration, audit write, object deletion, retention-deadline enforcement, legal-hold release command, raw payload retention, or compliance claim.                                                                                                                                                                                                                              |
| [OP_DOCUMENT_DISPOSITION_SCHEDULE_PROFILE_PROOF_2026-06-29.md](OP_DOCUMENT_DISPOSITION_SCHEDULE_PROFILE_PROOF_2026-06-29.md)                                                 | Row-local proof note | Proof for the bounded firm-configured disposition review schedule profile slice, covering nullable firm-settings persistence, staff-only read and `firm:update` write routes, safe audit metadata, workbench projection into `retentionHoldReview.dispositionMetadata.scheduleProfile`, Documents dashboard context rendering, selector-driven validation, and preserved no object deletion, deadline enforcement, legal-hold override, raw/export body retention, public/client controls, or compliance claim.                                                                                                                                                                                   |
| [OP_DOCUMENT_DISPOSITION_REVIEWER_PACKET_PROOF_2026-06-30.md](OP_DOCUMENT_DISPOSITION_REVIEWER_PACKET_PROOF_2026-06-30.md)                                                   | Row-local proof note | Proof for the staff-only matter-scoped disposition reviewer packet command, covering enum-only decisions over `retentionHoldReview.dispositionMetadata`, blocker rejection, safe audit metadata, Documents dashboard rendering, selector-driven validation, and preserved no object deletion, deadline enforcement, legal-hold release or override, raw OCR/provider/export body retention, public/client controls, or compliance claim.                                                                                                                                                                                                                                                          |
...
| [OP_MATTER_LIFECYCLE_REOPEN_BOUNDARY_PROOF_2026-06-27.md](OP_MATTER_LIFECYCLE_REOPEN_BOUNDARY_PROOF_2026-06-27.md)                                                           | Row-local proof note | Proof for the narrow status-only closed/archived matter lifecycle reopen boundary, covering `reopen` from `paused`, `closed`, and `archived` matters to `open`, latest-ready journal gating, safe audit metadata including `retentionChanged: false`, selector-driven validation, and preserved no-`closedOn`/portal/task/assignment/billing/trust/retention/cleanup/source-record side effects.                                                                                                                                                                                                                                                                                                  |
...
| [OP_REPORT_EXPORT_DOWNLOAD_AUDIT_EVENTS_PROOF_2026-06-29.md](OP_REPORT_EXPORT_DOWNLOAD_AUDIT_EVENTS_PROOF_2026-06-29.md)                                                     | Row-local proof note | Proof for safe report export download audit events, covering completed audit, billing, staff report, and jurisdictional trust export jobs, job/profile/scope/count metadata, explicit no-storage flags, billing export taxonomy de-duplication, selector-driven validation, and preserved no custom SQL, BI embeds, scheduled delivery, raw body storage, private audit payload exposure, export serialization/profile changes, payment mutation, trust posting, or certification claims.                                                                                                                                                                                                         |
| [OP_REPORT_EXPORT_PROFILE_ALIGNMENT_PROOF_2026-06-29.md](OP_REPORT_EXPORT_PROFILE_ALIGNMENT_PROOF_2026-06-29.md)                                                             | Row-local proof note | Proof for the read-only staff reporting export-profile alignment view, covering manual report export profile vs financial field-profile comparison metadata, bounded field-key samples, Reports dashboard rendering, selector-driven validation, and preserved no custom SQL, BI embeds, scheduled execution/email, raw body storage, payment processor exposure, payment creation/allocation, invoice mutation, trust posting, or certification claims.                                                                                                                                                                                                                                          |
| [OP_EXPENSE_CATEGORY_ACCOUNTING_EXPORT_PROFILE_PREVIEW_PROOF_2026-07-01.md](OP_EXPENSE_CATEGORY_ACCOUNTING_EXPORT_PROFILE_PREVIEW_PROOF_2026-07-01.md)                       | Row-local proof note | Proof for the read-only staff Reports workspace expense-category accounting export profile preview, covering existing firm category-code mappings, category counts, OP-authored review buckets/cues, dashboard rendering, selector-driven validation, and preserved no external accounting provider, export serialization change, invoice recalculation, payment mutation, trust posting, or certified-accounting claim.                                                                                                                                                                                                                                                                          |
...
| [OP_TRUST_CONTROLS_POLICY_PREVIEW_MATRIX_PROOF_2026-06-29.md](OP_TRUST_CONTROLS_POLICY_PREVIEW_MATRIX_PROOF_2026-06-29.md)                                                   | Row-local proof note | Proof for the read-only Trust Controls maker-checker policy preview matrix, covering additive category columns, safe matter rows, matter-scoped matrix cells, dashboard boundary copy, and preserved no-policy-enabled/no direct-posting-change/no approval or rejection mutation/no auto-posting/no settlement/no auto-match/no bank-feed connection/no jurisdiction-certified accounting boundaries.                                                                                                                                                                                                                                                                                            |

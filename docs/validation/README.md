...
conversion review, ready only for same-matter `ready_for_review` or `reviewed`
`document_analysis_status` artifacts and blocked for rejected, queued, failed, not-requested,
missing, draft, or otherwise not-ready states. The packet is limited to IDs, counts/lengths,
statuses, and fixed policy flags, while preserving no provider activation, schema, migration, queue,

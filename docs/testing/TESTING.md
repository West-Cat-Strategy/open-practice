...
is a successful blocker detection but still prevents a green automated release proof. Successful
external HTTPS S3 restore-drill evidence can accompany manual handoff for an external
object-storage path, and separate MinIO hardening proof is required when bundled MinIO remains the
release path. External S3 restore evidence uses an ignored operator env file and synthetic markers
only; it does not change residual-watch semantics for bundled MinIO deployments.

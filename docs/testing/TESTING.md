...
| Release readiness       | `pnpm release:local`                               | Creates a local release proof artifact with dependency audit, license JSON, SBOM, full local gate, migration replay, artifact secret scan, and diff whitespace evidence. Use `pnpm release:local -- --private-pilot` when a private self-hosted pilot also needs the restore drill. |
...
| Self-host restore drill or private-pilot release-proof tooling | `pnpm selfhost:restore-drill -- --env-file docker/selfhost.example.env --allow-synthetic-example`, `pnpm policy:check`, `pnpm test`                                                                                                                                                                                                                         |

...
| Security review tooling, Gitleaks fingerprint ignores, and secret scanners      | `pnpm security:review`, `pnpm security:secrets-history`, `pnpm policy:check`, `pnpm test`                                                                                                                                                                                                                                                                   |
...
Keep `.gitleaksignore` limited to exact reviewed Gitleaks fingerprints for synthetic historical
fixtures. Do not add broad rule, path, regex, or commit allowlists when tuning false positives; keep
`pnpm security:scan` as the high-confidence tracked-content gate.
...
remain non-green release proof. Successful external HTTPS S3 restore-drill evidence can accompany
manual handoff for an external object-storage path. External S3 restore evidence uses an ignored
operator env file and synthetic markers only.

...
| Secrets history scan     | `pnpm security:secrets-history`                    | Runs optional local Gitleaks history/diff scanning when installed. Uses exact reviewed fingerprints from `.gitleaksignore` for known synthetic proof/test false positives; findings outside those fingerprints remain review artifacts, not automatic publication blockers.                                                                                                                                                                        |
...
Compose services; Docker daemon unavailability is always a deterministic local-environment blocker,
even during the soft pre-build capacity probe. Add `-- --refresh` when the proof needs pinned Redis
pulls and `--pull` image rebuilds. Use `pnpm docker:app-smoke -- --keep-up` to validate and leave
the default dev stack running on web `33000` and API `34000`. For footprint work, pair the smoke result with
...
container filesystem by running `df -Pk /` in an already-local image with `--pull=never`. If Docker
is unreachable, it reports blocker code `docker_daemon_unavailable`, reason `docker_unreachable`,
and kind `local-environment`, with guidance to start Docker Desktop or set `DOCKER_HOST` to a
reachable daemon. It prefers local Open Practice images, then common local service images, then the
first non-dangling local image. If capacity is below the default 8 GiB threshold, reclaim Docker
storage and rerun the Docker validation command. The helper does not run `docker system prune` or
